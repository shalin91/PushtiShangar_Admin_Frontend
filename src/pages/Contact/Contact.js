...
const Contact = () => {
  const { GetContacts, Deletecontact, GetContactbyId } =
  ...
  const Getcontent = async () => {
    ...
    const transformedData = res.contacts.map((content, index) => ({
      ...content,
      id: index + 1,
    }));
    ...
    <>
                      ...
                      >
                        ...
                        <tbody className="list form-check-all">
                          {filteredSubCategories.length > 0
                            ? filteredSubCategories.map((content, key) => (
                                <tr key={content.id}>
      ...
      >
        ...
        <ModalBody>
          {viewedContent && (
            <div>
              <h5>{viewedContent.name}</h5>
              <p>Subject: {viewedContent.subject}</p>
              <p>Message: {viewedContent.message}</p>
            </div>
          )}
        </ModalBody>
      </Modal>
